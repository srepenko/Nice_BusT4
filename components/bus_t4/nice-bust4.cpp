...
void NiceBusT4::send_array_cmd (const uint8_t *data, size_t len) {
  ...
  //Serial1.flush();
  delayMicroseconds(120);                                          // добавляем задержку к ожиданию, иначе скорость переключится раньше отправки. С задержкой на d1-mini я получил идеальный сигнал, break = 520us
